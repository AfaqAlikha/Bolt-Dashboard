    ...
    const getAllUsers = async () => {
      ...
      try {
        ...
        if (snapshot.exists()) {
          ...
          const filteredUsers = Object.values(allUsers).filter(user => user.screenLock?.locked === false);
  ...
  return (
    <div>
      <div className="mt-3">
        ...
        <ActiveUserTable data={users} loading={loading} onDeleteUser={handleDeleteUser}  />
      </div>
    </div>
  );
};
